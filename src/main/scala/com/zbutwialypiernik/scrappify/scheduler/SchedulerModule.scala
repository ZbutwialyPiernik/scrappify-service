...
class SchedulerModule(databaseConfiguration: DatabaseConfiguration,
  ...
  def executeTask(productId: Int, url: AbsoluteUrl): Unit = {
    logger.info(s"Starting a new scrapping task for product $productId at $url")

    Await.result(scrappingTask.execute(productId, url)
      .value
      .andThen {
        case Success(Left(error: ServiceError)) => logger.error(s"Service error in scheduled scrapping task for product $productId at $url: ${error.message}")
