...
trait ErrorHandlers extends SLF4JLogging with CustomDirectives with JsonSupport {
  ...
  val rejectionHandler: RejectionHandler =
    RejectionHandler.newBuilder()
      .handle {
        case ModelValidationRejection(validationErrors) => complete(BadRequest, ValidationErrorResponse(BadRequest, "Request body is invalid", validationErrors.groupMap(_.path.asString)(_.message)))
      }
      .result()
